...
- Repo: FRC-2026
- Changes:
  - Fixed red alliance inverted controls: disabled `allianceRelativeControl` Ã¢â‚¬â€ was double-flipping with `zeroGyroWithAlliance()` causing inverted X/Y on red
  - Center modules moved from Back button to POV Down
  - Slow mode changed from binary 50% toggle (left trigger > 0.5) to progressive scaling (80% at no pull Ã¢â€ â€™ 20% at full pull)
  - Added right trigger progressive intake (0% Ã¢â€ â€™ 100% speed for intake roller + floor conveyor)
...
- Competition day fixes and tuning Ã¢â‚¬â€ numerous iterative deploys throughout the day
- Repo: FRC-2026
- Fixes applied:
...
- Files created:
  ...
  - `2026Robot/src/main/java/frc/robot/subsystems/TurretSubsystem.java` (2x SparkMax NEO 550 CAN 34-35, position PID, Ã‚Â±180Ã‚Â° soft limits)
...
- Files modified:
  ...
  - `2026Robot/src/main/deploy/swerve/modules/physicalproperties.json` (driveGearRatio 8.14, angleGearRatio 12.8 Ã¢â‚¬â€ MK4 L1 standard)
...
- Notes:
  ...
  - All motor inversions default to false Ã¢â‚¬â€ VERIFY AND SET ON HARDWARE before first drive
  - Turret seeds at zero on boot Ã¢â‚¬â€ manually align to forward before each enable, then press operator Start to zero encoders
  ...
  - Tier 2 D-pad presets: all angles/RPMs are placeholders Ã¢â‚¬â€ tune on hardware
