...
Robot.java (minimal entry point)
    â””â”€> RobotContainer.java (instantiation & bindings ONLY)
            â”œâ”€> Subsystems (hardware control)
            â”‚       â””â”€> Command Factories (preferred for simple logic)
            â””â”€> Commands (complex state logic)
```
...
**Launching Elastic:** VS Code â†’ `Ctrl+Shift+P` â†’ `WPILib: Start Tool` â†’ `Elastic`
...
â”œâ”€â”€ settings.json      # Robot configuration (max velocity, size)
â”œâ”€â”€ autos/             # Autonomous routines (.auto files)
â””â”€â”€ paths/             # Path segments (.path files)
```
...
| `Toggle Switch` | Enable/disable flags | â€” | Boolean |
...
| `PIDController` | PID tuning widgets | â€” | Sendable PIDController |
| `Field` | Robot position on field | `field_game`, `robot_width`, `robot_length` | Sendable Field2d |
| `SwerveDrive` | Module state visualization | â€” | Sendable |
| `Camera Stream` | Video feed | `url` | â€” |
| `ComboBox Chooser` | Mode selection | â€” | Sendable Chooser |
| `Network Alerts` | Fault reporting | â€” | Alerts topic |
| `Match Time` | Match countdown | â€” | â€” |
...
3. Use consistent grid positioning â€” no overlapping widgets
